...
export function usePagination<T = unknown>(
  ...
  // Clamp page when totalPages decreases (e.g., after filtering)
  useEffect(() => {
    if (page > totalPages) {
  ...
  const pageRange = useMemo(() => {
    ...
    if (start > 2) pages.push(-1); // Ellipsis marker (left)

    for (let i = start; i <= end; i++) {
      pages.push(i);
    }

    if (end < totalPages - 1) pages.push(-2); // Ellipsis marker (right)
